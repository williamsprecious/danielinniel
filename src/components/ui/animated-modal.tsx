  ...
  return (
    <>
      {React.Children.map(children, (child) => {
        // Pass open and setOpen to all children if they accept it
...
const Overlay = ({
  ...
  return (
    <motion.div
      ...
      className={`fixed inset-0 h-full w-full bg-background/80 z-50 ${className}`}
      onClick={onClick}
    ></motion.div>
  );
};
...
export const useOutsideClick = (
  ...
  useEffect(() => {
    const listener = (event: MouseEvent | TouchEvent) => {
      ...
      // Ignore clicks inside Radix UI Select dropdowns
      if (
